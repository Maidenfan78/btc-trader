/**
 * TCF2 Multi-Asset Bot Configuration
 *
...
loadEnvConfig('.env', process.env.BOT_ENV_FILE || '.env.tcf2');

let logger: Logger;

function getLogger(): Logger {
  if (!logger) {
    const logFile = process.env.BOT_LOG_FILE || 'logs/bot-tcf2.log';
    const errorFile = process.env.BOT_ERROR_LOG_FILE || 'logs/error-tcf2.log';
    logger = createLogger({
      botId: 'tcf2',
      logDir: 'logs',
      logLevel: 'info',
...
export function loadTCF2Config(): TCF2Config {
  const log = getLogger();
  log.info('Loading TCF2 bot configuration...');

  const config: TCF2Config = {
    // RPC
    ...
    // TCF2 Settings
    tcf2Period: getEnvNumber('TCF2_PERIOD', 35),
    tcf2T3Period: getEnvNumber('TCF2_T3_PERIOD', 3),
    tcf2SmoothFactor: getEnvNumber('TCF2_SMOOTH_FACTOR', 0.7),

    // MFI placeholder (not used by TCF2 but required by interface)
    mfiPeriod: 14,
    mfiBuyLevel: 20,
    mfiSellLevel: 80,
  ...
  // Validation
  validateTCF2Config(config, log);

  log.info('TCF2 bot configuration loaded successfully');
...
function validateTCF2Config(config: TCF2Config, log: Logger): void {
  // TCF2 validation
  if (config.tcf2Period < 1 || config.tcf2Period > 200) {
    throw new Error(`TCF2_PERIOD must be between 1-200, got ${config.tcf2Period}`);
  }
  if (config.tcf2T3Period < 1 || config.tcf2T3Period > 50) {
    throw new Error(`TCF2_T3_PERIOD must be between 1-50, got ${config.tcf2T3Period}`);
  }
  if (config.tcf2SmoothFactor <= 0 || config.tcf2SmoothFactor >= 1) {
    throw new Error(`TCF2_SMOOTH_FACTOR must be between 0-1, got ${config.tcf2SmoothFactor}`);
  }
...
export { getLogger as getTCF2Logger };
